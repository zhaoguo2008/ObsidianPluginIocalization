...
var TemplaterSettingTab = class extends import_obsidian6.PluginSettingTab {
  ...
  add_template_folder_setting() {
    new import_obsidian6.Setting(this.containerEl).setName("模板文件夹位置").setDesc("此文件夹中的文件将作为模板可用。").addSearch((cb) => {
      ...
      cb.setPlaceholder("示例：Folder1/Folder2").setValue(this.plugin.settings.templates_folder).onChange((new_folder) => {
  ...
  add_internal_functions_setting() {
    ...
    desc.append("Templater提供了您可以使用的多个预定义变量/函数。", desc.createEl("br"), "检查 ", desc.createEl("a", {
  ...
  add_syntax_highlighting_settings() {
    ...
    desktopDesc.append("在编辑模式下为Templater命令添加语法突出显示。");
    const mobileDesc = document.createDocumentFragment();
    mobileDesc.append("在移动设备的编辑模式下为Templater命令添加语法突出显示。谨慎使用：这可能会破坏移动平台上的实时预览。");
    new import_obsidian6.Setting(this.containerEl).setName("在桌面上突出显示语法").setDesc(desktopDesc).addToggle((toggle) => {
    ...
    new import_obsidian6.Setting(this.containerEl).setName("在 Mobile 上突出显示语法").setDesc(mobileDesc).addToggle((toggle) => {
  ...
  add_auto_jump_to_cursor() {
    const desc = document.createDocumentFragment();
    desc.append("自动触发 ", desc.createEl("code", { text: "tp.file.cursor" }), " 在插入模板之后。", desc.createEl("br"), "您还可以将热键设置为手动触发", desc.createEl("code", { text: "tp.file.cursor" }), ".");
  ...
  add_trigger_on_new_file_creation_setting() {
    const desc = document.createDocumentFragment();
    desc.append("Templater将监听新文件创建事件，并替换在新文件内容中找到的每个命令。", desc.createEl("br"), "这使得Templater与其他插件兼容，如Daily Note核心插件、日历插件、审查插件、Note重构插件 ...", desc.createEl("br"), desc.createEl("b", {
      text: "Warning: "
    }), "如果在创建时创建包含未知/不安全内容的新文件，这可能很危险。确保每个新文件的内容在创建时都是安全的。");
    new import_obsidian6.Setting(this.containerEl).setName("创建新文件时触发模板").setDesc(desc).addToggle((toggle) => {
  ...
  add_ribbon_icon_setting() {
    const desc = document.createDocumentFragment();
    desc.append("在侧边栏功能区中显示模板图标，允许您在任何地方快速使用模板。");
    new import_obsidian6.Setting(this.containerEl).setName("在侧边栏中显示图标").setDesc(desc).addToggle((toggle) => {
  ...
  add_templates_hotkeys_setting() {
    ...
    this.plugin.settings.enabled_templates_hotkeys.forEach((template, index) => {
      const s = new import_obsidian6.Setting(this.containerEl).addSearch((cb) => {
        ...
        cb.setPlaceholder("示例: folder1/template_file").setValue(template).onChange((new_template) => {
    ...
    new import_obsidian6.Setting(this.containerEl).addButton((cb) => {
      cb.setButtonText("为模板添加新的热键").setCta().onClick(() => {
  ...
  add_folder_templates_setting() {
    ...
    descHeading.append("文件夹模板在新的 ", descHeading.createEl("strong", { text: "empty " }), "在给定的文件夹中创建文件。", descHeading.createEl("br"), "Templater将用指定的模板填充空文件.", descHeading.createEl("br"), "使用了最深的匹配项。将在根目录上定义一个全局默认模板 ", descHeading.createEl("code", { text: "/" }), ".");
    ...
    descUseNewFileTemplate.append("启用后，Templater将使用下面定义的文件夹模板。");
    ...
    new import_obsidian6.Setting(this.containerEl).setName("添加新内容").setDesc("添加新文件夹模板").addButton((button) => {
      button.setTooltip("添加其他文件夹模板").setButtonText("+").setCta().onClick(() => {
      ...
      const s = new import_obsidian6.Setting(this.containerEl).addSearch((cb) => {
        ...
        cb.setPlaceholder("Folder").setValue(folder_template.folder).onChange((new_folder) => {
          if (new_folder && this.plugin.settings.folder_templates.some((e) => e.folder == new_folder)) {
            log_error(new TemplaterError("此文件夹已有一个与其关联的模板"));
            return;
          }
  ...
  add_startup_templates_setting() {
    ...
    const desc = document.createDocumentFragment();
    desc.append("启动模板是在Templater启动时执行一次的模板。", desc.createEl("br"), "这些模板不会输出任何内容.", desc.createEl("br"), "这在设置模板时非常有用，例如，将挂钩添加到黑玉石事件.");
    new import_obsidian6.Setting(this.containerEl).setDesc(desc);
  ...
  add_user_script_functions_setting() {
    ...
    desc.append("此文件夹中的所有JavaScript文件都将作为CommonJS模块加载，以导入自定义用户函数.", desc.createEl("br"), "需要可以从存储库访问该文件夹.", desc.createEl("br"), "检查一下 ", desc.createEl("a", {
      href: "https://silentvoid13.github.io/Templater/",
      text: "documentation"
    }), " 了解更多信息.");
    new import_obsidian6.Setting(this.containerEl).setName("脚本文件文件夹位置").setDesc(desc).addSearch((cb) => {
      new FolderSuggest(cb.inputEl);
      cb.setPlaceholder("示例: folder1/folder2").setValue(this.plugin.settings.user_scripts_folder).onChange((new_folder) => {
  ...
  add_user_system_command_functions_setting() {
    let desc = document.createDocumentFragment();
    desc.append("允许您创建链接到系统命令的用户函数.", desc.createEl("br"), desc.createEl("b", {
      text: "警告: "
    }), "从不受信任的来源执行任意系统命令可能很危险。仅运行您理解的、来自可信来源的系统命令.");
    this.containerEl.createEl("h2", {
      text: "User System Command Functions"
    });
    new import_obsidian6.Setting(this.containerEl).setName("启用用户系统命令功能").setDesc(desc).addToggle((toggle) => {
    ...
    if (this.plugin.settings.enable_system_commands) {
      ...
      desc = document.createDocumentFragment();
      desc.append("用于执行命令的外壳二进制文件的完整路径.", desc.createEl("br"), "此设置是可选的，如果未指定，则将默认为系统的默认shell.", desc.createEl("br"), "如果有疑问，可以在所有平台上使用正斜杠(‘/’)作为路径分隔符.");
      new import_obsidian6.Setting(this.containerEl).setName("外壳二进制位置").setDesc(desc).addText((text) => {
  ...
  add_donating_setting() {
    const s = new import_obsidian6.Setting(this.containerEl).setName("捐赠").setDesc("如果你喜欢这个插件，考虑捐献以支持持续发展.");
