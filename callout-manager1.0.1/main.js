...
var SelectIconPane = class extends UIPane {
  ...
  display() {
    ...
    if (previewLimitOverage > 0) {
      ...
      containerEl.createEl("p", {
        text: `那里 ${pluralIs} ${previewLimitOverage} 更多 ${pluralIcon} 以显示。优化搜索以查看更多信息.`
      });
    }
  }
  /** @override */
  displayControls() {
    ...
    new import_obsidian10.TextComponent(controlsEl).setValue(this.searchQuery).setPlaceholder("搜索图标...").onChange(this.search.bind(this));
  }
  /** @override */
  onReady() {
    this.resetSearchResults();
...
var CalloutIconSetting = class extends import_obsidian11.Setting {
  constructor(containerEl, callout, plugin, getNav) {
    ...
    this.addButton((btn) => {
      ...
      btn.onClick(() => {
        getNav().open(
          new SelectIconPane(plugin, "选择图标", { onChoose: (icon) => {
            var _a;
...
var PerSchemeAppearanceEditor = class extends AppearanceEditor {
  ...
  render() {
    ...
    new CalloutColorSetting(containerEl, callout).setName("暗色").setDesc("更改深色方案的详图索引的颜色.").setColorString(colorDark).onChange((color) => this.setAppearanceOrChangeToUnified({ ...appearance, colorDark: color }));
    new CalloutColorSetting(containerEl, callout).setName(`亮色`).setDesc(`更改灯光配色方案的详图索引的颜色.`).setColorString(colorLight).onChange((color) => this.setAppearanceOrChangeToUnified({ ...appearance, colorLight: color }));
    new CalloutIconSetting(containerEl, callout, plugin, () => nav).setName("图标").setDesc("更改详图索引图标.").setIcon(otherChanges.icon).onChange(
...
var UnifiedAppearanceEditor = class extends AppearanceEditor {
  ...
  render() {
    ...
    new CalloutColorSetting(containerEl, callout).setName("颜色").setDesc("更改详图索引的颜色.").setColorString(color).onChange((color2) => setAppearance({ type: "unified", otherChanges, color: color2 }));
    new import_obsidian12.Setting(containerEl).setName(`配色方案`).setDesc(`更改的详图索引的颜色 ${otherColorScheme} 配色方案.`).addButton(
    ...
    new CalloutIconSetting(containerEl, callout, plugin, () => this.nav).setName("图标").setDesc("更改详图索引图标.").setIcon(otherChanges.icon).onChange((icon) => setAppearance({ type: "unified", color, otherChanges: { ...otherChanges, icon } }));
  }
};
...
function renderInfo(app2, callout, containerEl) {
  ...
  contentEl.createEl("h2", { text: "关于此标注" });
  contentEl.createEl("div", { cls: "calloutmanager-edit-callout-info" }, (el) => {
    ...
    if (callout.sources.length === 1) {
      if (callout.sources[0].type === "builtin") {
        el.appendText(". 它是内置的callout之一.");
        return;
      }
      el.appendText(". 它是由添加到黑石中的 ");
...
var ManageCalloutsPane = class extends UIPane {
  ...
  displayControls() {
    const { controlsEl } = this;
    new import_obsidian16.TextComponent(controlsEl).setValue(this.searchQuery).setPlaceholder("过滤器标注...").onChange(this.search.bind(this));
    if (!this.viewOnly) {
...
var ManagePluginPane = class extends UIPane {
  ...
  display() {
    ...
    new import_obsidian17.Setting(containerEl).setName("主题").setDesc("查找主题提供的标注。").addToggle((setting) => {
    ...
    new import_obsidian17.Setting(containerEl).setName("代码段").setDesc("在自定义CSS代码段中查找标注.").addToggle((setting) => {
    ...
    containerEl.createEl("h2", { text: "重置" });
    new import_obsidian17.Setting(containerEl).setName("重置标注设置").setDesc("重置对标注所做的所有更改.").addButton(
      withConfirm((btn) => {
    ...
    new import_obsidian17.Setting(containerEl).setName("重置自定义标注").setDesc("删除您创建的所有自定义标注.").addButton(
      withConfirm((btn) => {
        btn.setButtonText("重置").onClick(() => {
